...
raw_transcript <- read_csv(here("import/dahan2025_hedging/raw_data/segmented_transcript.csv"))
...
choices_info <- read_tsv(here("import/dahan2025_hedging/raw_data/Alignment_Data.txt")) |>
...
all <- text |>
  ...
  mutate(
    ...
    dataset_id = "dahan2025_hedging",
    full_cite = "Dahan, D. (2025). When hedging helps, rather than impedes, communication: collaboration in the referential communication task. Discourse Processes, 62(2), 89-111.",
    short_cite = "Dahan (2025)",
    group_size = 2,
    language = "English",
