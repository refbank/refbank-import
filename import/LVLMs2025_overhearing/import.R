...
# Basic dataset info
dataset_id <- "LVLMs2025_overhearing"
...
# File paths
baskets_file <- "refbank-import/import/LVLMs2025_overhearing/raw_data/baskets-matching-data-plus-formal.xlsx"
dogs_file    <- "refbank-import/import/LVLMs2025_overhearing/raw_data/dogs-matching-data-plus-formal.xlsx"
...
harmonize_matching_dataset <- function(df, condition_label) {
  ...
  df <- df %>%
    mutate(item_num = as.numeric(str_extract(Order, "\\d+")),
  ...
  if (nrow(missing_describer_trials) > 0) {
    cat(sprintf("Warning: %d trials missing describer, adding placeholder messages\n", 
    ...
    for (i in 1:nrow(missing_describer_trials)) {
      ...
      orig_row <- df_long %>%
        filter(paste0(condition_label, "_", player) == trial_info$game_id,
