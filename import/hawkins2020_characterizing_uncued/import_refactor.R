...
extract_image_number <- function(text, type) {
  case_when(
    type %in% c("numbered", "colon", "dash", "standalone", "ordinal",
...
boards_dict <- read_csv(BOARD_FILE) %>%
  ...
  mutate(source=str_sub(type, 1, -2),
...
low_accuracy_games <- boards_dict |> select(gameid, repetitionNum, score) |> 
  unique() |> group_by(gameid) |> add_tally() |> 
  ungroup() |> filter(n==6) |> 
  filter(score<8) |> 
  group_by(gameid) |> tally() |> filter(n>3) |> 
  pull(gameid)
