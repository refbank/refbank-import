...
role_parity <- raw |> mutate(rep_parity=rep_no%%2) |> filter(role=="director") |>  select(game_no, participant, role, rep_parity) |> distinct()

roles <- role_parity |> mutate(rep_parity=(rep_parity+1)%%2, role="matcher") |> bind_rows(role_parity)

choices <- raw |> select(game_no, round_no, rep_no, target, response) |> 
  mutate(
    choice_id=case_when(
...
messages <- raw |> 
  ...
  mutate(action_type="message",
         role=case_when(role=="director" ~ "describer",
                        T ~ role)) |> 
...
missing_messages <- choices |> distinct(game_no, round_no, rep_no, target) |> anti_join(
  messages |> filter(role=="describer") |> distinct(game_no, round_no, rep_no, target)) |> 
  mutate(action_type="message", rep_parity=rep_no%%2) |> mutate(role="director") |> left_join(roles) |> 
  mutate(role="describer") |> select(-rep_parity)

all <- choices |> bind_rows(messages) |> bind_rows(missing_messages) |> 
  mutate(condition_label="codeswitch",
  ...
  rename(game_id=game_no, player_id=participant) |> select(-round_no, -rep_no)
