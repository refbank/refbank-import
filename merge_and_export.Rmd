...
na_trials <- trials |> filter(if_any(c("condition_id", "game_id", "room_num", "option_set", "target", "stage_num", "trial_num", "rep_num", "describer", "dataset_id", "order_match"), is.na))
assert_that(nrow(na_trials) == 0)

trials <- trials |> mutate(option_size = option_set |> str_split(";") |>
  lengths())

assert_that(all(trials$trial_num > 0))
assert_that(all(trials$rep_num > 0))
assert_that(all(trials$stage_num > 0))


assert_that(max(trials$option_size) == 36) # change these as datasets change, but we're trying to rule out screw ups that make it like 96 or something absurd
assert_that(min(trials$option_size) == 2)
...
trials |> select(target, dataset_id) |> unique()  |> anti_join(images |> select(target = image_id))
options <- trials |>
  select(option_set) |>
  mutate(image_id = str_split(option_set, ";")) |>
  unnest(image_id) |>
  select(image_id) |>
  unique()
